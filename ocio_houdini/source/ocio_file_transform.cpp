...
bool Ocio_file_transform::updateParmsFlags()
{
	...
	//Set attrs. invisible
	attribute_change_occured |= setVisibleState(ocio_file_transform_parameters::prm_last_lut_file_path.getToken(), ocio_houdini_constants::INTERNAL_PARMS_VISIBLE);
	attribute_change_occured |= setVisibleState(ocio_file_transform_parameters::prm_last_cccid.getToken(), ocio_houdini_constants::INTERNAL_PARMS_VISIBLE);
	attribute_change_occured |= setVisibleState(ocio_file_transform_parameters::prm_last_direction.getToken(), ocio_houdini_constants::INTERNAL_PARMS_VISIBLE);
	attribute_change_occured |= setVisibleState(ocio_file_transform_parameters::prm_last_interpolation.getToken(), ocio_houdini_constants::INTERNAL_PARMS_VISIBLE);
	
	return attribute_change_occured;
}

//disableParms
unsigned Ocio_file_transform::disableParms()
{
	//time
	float time = OCIO_houdini_functionality.get_time();
											...
											COP2_Node* node)
{
	...
	((Ocio_file_transform*)node)->OCIO_houdini_functionality.filter(context, 
																	input, 
																	output, 
																	((Ocio_file_transform*)node)->processor);

	//Return error
	return ((Ocio_file_transform*)node)->error();
};
